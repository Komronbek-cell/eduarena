...
import { ArrowLeft, Zap, ChevronRight, Loader2, Clock, CheckCircle, Lock, Filter } from 'lucide-react'
...
export default function DailyQuizzesPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const [{ data: quizzes }, { data: attempts }] = await Promise.all([
        supabase.from('quizzes').select('*').eq('type', 'daily').order('created_at', { ascending: false }),
  ...
  const activeCount = quizzes.filter(q => q.status === 'active' && !completedIds.includes(q.id) && (!q.deadline || new Date(q.deadline) > new Date())).length
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900">
      <nav className="bg-white border-b border-gray-100 px-4 py-4 sticky top-0 z-10">
        <div className="max-w-3xl mx-auto flex items-center gap-3">
          ...
          <span className="font-black text-base md:text-lg">Kunlik quizlar</span>
      ...
      <main className="max-w-3xl mx-auto px-4 py-6 md:py-8">
        ...
        <div className="grid grid-cols-3 gap-3 mb-5">
          <div className="bg-white border border-gray-100 rounded-2xl p-3 md:p-4 text-center shadow-sm">
            <p className="text-xl md:text-2xl font-black text-violet-600">{quizzes.length}</p>
        ...
        {filtered.length === 0 ? (
          <div className="bg-white border border-gray-100 rounded-2xl p-10 text-center shadow-sm">
            ...
            <p className="text-gray-400 text-sm">Bu bo'limda quiz yo'q</p>
          </div>
        ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap mb-1.5">
                        ...
                        {!isCompleted && isActive && !isExpired && (
                          <span className="text-xs bg-violet-100 text-violet-700 px-2 py-0.5 rounded-full font-bold">Faol</span>
                        )}
