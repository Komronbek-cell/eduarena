...
import {
  ...
  UserX, UserCheck, Crown, MoreVertical, X, Image as ImageIcon
} from 'lucide-react'
...
export default function StudentsPage() {
  ...
  const toggleBlock = async (student: Student) => {
    if (student.id === currentUserId) {
      alert("O'z akkauntingizni bloklash mumkin emas!")
  ...
  const toggleAdmin = async (student: Student) => {
    if (student.id === currentUserId) {
      alert("O'z adminligingizni olib tashlash mumkin emas!")
      setActionMenuId(null)
      return
    }
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900">
      ...
      <main className="max-w-6xl mx-auto px-4 md:px-6 py-6 md:py-8">
        ...
        <div className="grid grid-cols-3 gap-3 md:gap-4 mb-5">
          {[
            { label: 'Talabalar', value: studentCount, color: 'text-violet-600' },
            { label: 'Adminlar', value: adminCount, color: 'text-orange-500' },
            { label: 'Bloklangan', value: blockedCount, color: 'text-red-500' },
          ].map((s, i) => (
        ...
        <div className="flex items-center gap-2 mb-5 flex-wrap">
          <div className="flex gap-1.5">
            {[{ key: 'all', label: 'Barchasi' }, { key: 'student', label: 'Talabalar' }, { key: 'admin', label: 'Adminlar' }].map(f => (
              <button key={f.key} onClick={() => setFilterRole(f.key as any)}
                className={`text-xs font-bold px-3 py-1.5 rounded-xl transition ${filterRole === f.key ? 'bg-violet-600 text-white' : 'bg-white border border-gray-200 text-gray-500'}`}>
                {f.label}
              </button>
            ))}
          </div>
          <div className="flex gap-1.5 ml-auto">
            {[{ key: 'all', label: 'Hammasi' }, { key: 'active', label: 'Faol' }, { key: 'blocked', label: 'Bloklangan' }].map(f => (
              <button key={f.key} onClick={() => setFilterStatus(f.key as any)}
                className={`text-xs font-bold px-3 py-1.5 rounded-xl transition ${filterStatus === f.key ? 'bg-red-500 text-white' : 'bg-white border border-gray-200 text-gray-500'}`}>
                {f.label}
              </button>
            ))}
          </div>
        ...
        <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm">
          ...
          {filtered.length === 0 ? (
            ...
            return (
              <div key={student.id} className={`px-4 md:px-6 py-4 border-b border-gray-50 last:border-0 transition ${
                student.is_blocked ? 'bg-red-50/50' : isMe ? 'bg-violet-50/50' : 'hover:bg-gray-50'
              }`}>
                <div className="flex items-center gap-3 md:grid md:grid-cols-12 md:gap-4">
                  ...
                  <div className="col-span-4 flex items-center gap-3 flex-1 min-w-0">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        {isMe && <span className="text-xs bg-violet-100 text-violet-600 px-1.5 py-0.5 rounded-full font-bold flex-shrink-0">Siz</span>}
                        {student.role === 'admin' && (
                        ...
                        {student.is_blocked && (
                          <span className="text-xs bg-red-100 text-red-600 px-1.5 py-0.5 rounded-full font-bold flex-shrink-0">Bloklangan</span>
                        )}
                      </div>
                  ...
                  <div className="hidden md:flex col-span-3 items-center">
                    {editingId === student.id ? (
                      ...
                      <button onClick={() => setEditingId(student.id)}
                        ...
                        <span className={`px-2 py-1 rounded-lg font-semibold ${student.groups ? 'bg-violet-50 text-violet-700' : 'bg-gray-100 text-gray-400'}`}>
                  ...
                  <div className="hidden md:flex col-span-2 items-center justify-center">
                    <div className="flex items-center gap-1 text-violet-600 font-black text-sm">
                      <Trophy className="w-3.5 h-3.5" />{student.total_score}
                    </div>
                  </div>
                  ...
                  <div className="col-span-2 flex items-center justify-end gap-2 relative">
                    ...
                    <div className="relative">
                      ...
                      {actionMenuId === student.id && (
                        <div className="absolute right-0 top-8 bg-white border border-gray-100 rounded-xl shadow-lg z-20 min-w-[180px] overflow-hidden">
                          <button
                            onClick={() => toggleAdmin(student)}
                            disabled={isMe}
                            className="w-full flex items-center gap-2.5 px-4 py-3 text-sm hover:bg-gray-50 disabled:opacity-40 disabled:cursor-not-allowed transition text-left"
                          >
                            {student.role === 'admin'
          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-2xl">
            ...
            <div className="space-y-3">
              <div>
                ...
                <input
                  type="number" value={scoreAmount} onChange={e => setScoreAmount(e.target.value)}
                  placeholder="Masalan: 50" min={1} autoFocus
              ...
              {scoreAmount && (
                <div className={`text-sm font-semibold rounded-xl px-4 py-3 ${scoreModal.type === 'add' ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-600'}`}>
              ...
              <div className="flex gap-3 pt-1">
                ...
                <button onClick={handleScoreChange} disabled={saving || !scoreAmount}
                  className={`flex-1 py-3 rounded-xl text-white font-black text-sm transition ${
                    scoreModal.type === 'add' ? 'bg-green-500 hover:bg-green-600 disabled:opacity-50' : 'bg-red-500 hover:bg-red-600 disabled:opacity-50'
                  }`}>
